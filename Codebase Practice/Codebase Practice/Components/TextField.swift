...
/* [주석-2]
 ...
 🙋🏻‍♀️ 야곰 책 18.3.6 요구 이니셜라이저
  - 상속받을 때 자식클래스에서 반드시 재정의해야 하는 이니셜라이저 앞에는 required를 붙인다.
  - 재정의할 때도 required를 붙인다.(평소엔 override 쓸 자리에)
 
 🙋🏻‍♀️ NSCoder
  - 추상클래스
  - 어떤 객체가 다른 객체를 아카이빙/배포(분배)할 수 있게 함
  - Coder Clases(NSCoder를 구현하는 클래스들): NSArchiver, NSUnArchiver,...
    이들의 인스턴스를 Codier Objects, Coders라고 함
 ...
 🙋🏻‍♀️ init(coder:)
    스토리보드에서 TextField 호출할 때 사용됨.
    스토리보드나 nib 파일을 통해 뷰를 로드하면서 커스텀하게 초기화해주고 싶을 때 사용.
    커스텀 클래스를 만드는 게 커스텀하게 초기화해주고 싶은 거니까 init(coder:)을 쓰는 것.
 
 ➕ Nib 파일
    Interface Builder 문서.
    이 문서가 수정 후 저장되면 객체그래프가 생성되어 저장됨.
    이 문서를 로드하면 객체그래프가 언아카이빙(아카이빙하기 전 객체 그래프를 복원)됨.
 ➕ 객체그래프의 모든 객체가 인스턴스화되고 초기화되고 나서 outlet, action 연결됨. 그리고 awakeFromNib이 불림.
 */
